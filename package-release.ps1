...
$stage = Join-Path $artifacts 'KellysAIRLIFT-0.14.0'
$resolvedProject = [System.IO.Path]::GetFullPath($projectRoot)
$resolvedStage = [System.IO.Path]::GetFullPath($stage)
if (-not $resolvedStage.StartsWith($resolvedProject + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
...
$examplesTarget = Join-Path $stage 'examples'
New-Item -ItemType Directory -Path $pluginTarget, $docsTarget, $examplesTarget -Force | Out-Null
Copy-Item -LiteralPath (Join-Path $projectRoot 'bin\Release\net472\KellysAIRLIFT.dll') -Destination $pluginTarget
Copy-Item -LiteralPath (Join-Path $projectRoot 'config\kelly.nuclearoption.airlift.cfg') `
    -Destination (Join-Path $examplesTarget 'kelly.nuclearoption.airlift.default.cfg')
Copy-Item -LiteralPath (Join-Path $projectRoot 'config\kelly.nuclearoption.airlift.server.cfg') -Destination $examplesTarget
Copy-Item -LiteralPath (Join-Path $projectRoot 'config\kelly.nuclearoption.airlift.zones.tsv') `
    -Destination (Join-Path $examplesTarget 'kelly.nuclearoption.airlift.zones.tsv')
...
$zip = Join-Path $artifacts 'KellysAIRLIFT-0.14.0-NO034.zip'
